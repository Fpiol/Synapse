...
export default function Menu({ isOpen, onClose, onNavigate, isLoggedIn, userInfo }: MenuProps) {
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="absolute left-0 top-0 bottom-0 w-[280px] bg-[#426b1f] z-50 overflow-hidden flex flex-col"
          >
...
function MenuItems({ onNavigate }: MenuItemsProps) {
  const menuItems = [
    { label: "商店", screen: "list" },
    { label: "新闻亭", screen: "newsstand" },
    { label: "关于我们", screen: "about" },
    { label: "购物篮", screen: "cart" },
  ];
...
function UserInfoSection({ isLoggedIn, userInfo, onNavigate }: UserInfoSectionProps) {
  if (!isLoggedIn) {
    // Show login button when not logged in
    return (
      <button
        onClick={() => onNavigate("login")}
        className="absolute bottom-6 left-4 right-4 bg-white/10 backdrop-blur-sm hover:bg-white/20 transition-all rounded-xl p-4 border border-white/20"
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 text-left">
            <p className="text-white font-['Inter:Medium',_sans-serif] font-medium text-[16px]">
              登录/注册
            </p>
            <p className="text-white/70 font-['Inter:Regular',_sans-serif] text-[12px]">
              查看订单和资料
            </p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-3">
        {userInfo?.avatarUrl ? (
          ...
          <div className="w-12 h-12 bg-white/90 rounded-full flex items-center justify-center">
            <span className="text-[#426b1f] font-['Inter:Bold',_sans-serif] font-bold text-[20px]">
              {userInfo?.fullName.charAt(0).toUpperCase()}
            </span>
          </div>
        )}
        <div className="flex-1 text-left">
          <p className="text-white font-['Inter:Medium',_sans-serif] font-medium text-[16px] truncate">
            {userInfo?.fullName}
          </p>
          <p className="text-white/70 font-['Inter:Regular',_sans-serif] text-[12px] truncate">
            {userInfo?.email}
          </p>
        </div>
      </div>
