...
export default function CheckoutPage({ cartCount, customerInfo, onBack, onMenuClick, onProceedToPayment, siteTitle }: CheckoutPageProps) {
  ...
  return (
    <div className="bg-[#ffffff] relative size-full">
      ...
      <Header 
        ...
        siteTitle={siteTitle || 'World Peas'}
      />
      ...
      <div className="absolute left-6 right-6 top-[286px]">
        <div className="box-border content-stretch flex flex-col gap-2 items-start justify-start p-0 relative w-full">
          ...
          <FormField label="State">
            <div className="relative w-full">
              <button
                onClick={() => formData.country === '美国' && setShowStateDropdown(!showStateDropdown)}
                disabled={formData.country !== '美国'}
                className={`bg-[#ffffff] border border-neutral-200 rounded-lg px-4 py-2 w-full font-['Inter:Regular',_sans-serif] text-[16px] flex items-center justify-between ${
                  formData.country !== '美国' ? 'text-[#757575] cursor-not-allowed' : 'text-[#000000]'
                }`}
              >
              ...
              {showStateDropdown && formData.country === '美国' && (
                <div className="absolute top-full left-0 right-0 bg-white border border-neutral-200 rounded-lg mt-1 max-h-48 overflow-y-auto z-50">
                  {US_STATES.map((state) => (
                    <button
                      key={state}
            ...
            <div className="relative w-full">
              ...
              {showZipCodeError && (
                <div className="mt-1 text-red-500 text-[12px] font-['Inter:Regular',_sans-serif]">
                  Please enter a valid {formData.country === '美国' ? 'zip code' : 
                                      formData.country === '加拿大' ? 'postal code' :
                                      formData.country === '英国' ? 'postcode' :
...
function Header({ cartCount, onBack, onMenuClick, siteTitle }: HeaderProps) {
  return (
